...
AnimationReference :: struct {
	...
	SDL_Texture *get_texture(animationReference: ^AnimationReference) -> ^SDL_Texture {
...
Animation :: struct {
	...
	get_frame :: proc(x: int, y: int, w: int, h: int) -> (x: int, y: int, w: int, h: int) {
	...
	update :: proc(animation: ^Animation, delta: u32) {
		animation->animation_counter = (this->animation_counter + delta) % this->wrap_around_value;
	}
